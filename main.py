...
def calculate_pocket_features(atoms):
    ...
    return {
        ...
        "Phosphate_Atoms":phosphate_atoms,
...
def visualize_structure(pdb_path,center=None,phosphate_atoms=None):
    ...
    if phosphate_atoms:
        ...
        for a in phosphate_atoms:

            coord=a.coord

            view.addSphere({
                "center":{"x":float(coord[0]),
                          "y":float(coord[1]),
                          "z":float(coord[2])},
                "radius":0.6,
                "color":"red"
...
if pdb_file:
    ...
    if ligands:
        ...
        st.sidebar.success(
            f"Pocket atoms detected: {len(p_atoms)}"
        )
...
if pdb_file and st.session_state.pocket_features:

    st.subheader("3D RNA Pocket View")

    show_phosphate = st.checkbox("Show Phosphate Sites")

    center = st.session_state.pocket_features["Center"]

    phos_atoms=None

    if show_phosphate:
        phos_atoms=st.session_state.pocket_features["Phosphate_Atoms"]

    view = visualize_structure("temp_struct.pdb",
                               center=center,
                               phosphate_atoms=phos_atoms)
    ...
    with col1:
        st.metric("Pocket Radius (Rg)",
                  f"{st.session_state.pocket_features['Pocket_Rg']:.2f} Å")

    with col2:
        st.metric("Phosphate Sites",
                  len(st.session_state.pocket_features["Phosphate_Atoms"]))

    with col3:
        st.metric("Polar Atoms",
                  len(st.session_state.pocket_features["Polar_Atoms"]))
